...
@interface WXMVideoCompression()
@property (nonatomic,assign) NSInteger width;
@property (nonatomic,assign) NSInteger height;
@end

@implementation WXMVideoCompression

- (instancetype)init {
    self = [super init];
    if (self) {
        _audioConfigurations.samplerate = WXMAudioSampleRate_11025Hz;
        _audioConfigurations.bitrate = WXMAudioBitRate_32Kbps;
        _audioConfigurations.numOfChannels = 1;
        _audioConfigurations.frameSize = 8;

        _videoConfigurations.fps = 15;
        _videoConfigurations.videoBitRate = WXM_VIDEO_BITRATE_LOW;
        _videoConfigurations.videoResolution = WXM_VIDEO_RESOLUTION_SUPER;
    }
    return self;
}
...
- (void)startCompressionWithCompletionHandler:(void (^)(WXMVideoCompressionState State))handler
{
    ...
    [self exportAsynchronouslyWithCompletionHandler:^{
        if ([self status] == AVAssetExportSessionStatusCompleted) {
            handler(WXM_VIDEO_STATE_SUCCESS); /** 成功 */
        } else if (self.status == AVAssetExportSessionStatusExporting) {
        } else {
            handler(WXM_VIDEO_STATE_FAILURE); /** 失败 */
        }
    }];
}
...
- (float)getVideoConfigurationsBitRate
{
    ...
    switch (self.videoConfigurations.videoResolution) {
        case WXM_VIDEO_RESOLUTION_LOW:
            self.height = 352;
            self.width = 288;
            switch (self.videoConfigurations.videoBitRate) {
                case WXM_VIDEO_BITRATE_LOW:
                    bitRate =(352 * 288 * 3)/4;
                    break;
                case WXM_VIDEO_BITRATE_MEDIUM:
                    
                    bitRate =(352 * 288 * 3)/2;
                    
                    break;
                case WXM_VIDEO_BITRATE_HIGH:
                    bitRate =(352 * 288 * 2);
                    
                    break;
                case WXM_VIDEO_BITRATE_SUPER:
                    bitRate =(352 * 288 * 3);
                    
                    break;
                case WXM_VIDEO_BITRATE_SUPER_HIGH:
                    bitRate =(352 * 288 * 3) * 2;
            ...
            break;
        case WXM_VIDEO_RESOLUTION_MEDIUM:
            self.height = 480;
            self.width = 360;
            switch (self.videoConfigurations.videoBitRate) {
                case WXM_VIDEO_BITRATE_LOW:
            ...
            break;
        case WXM_VIDEO_RESOLUTION_HIGH:
            self.height = 640;
            self.width = 480;
            switch (self.videoConfigurations.videoBitRate) {
                case WXM_VIDEO_BITRATE_LOW:
            ...
            break;
        case WXM_VIDEO_RESOLUTION_SUPER:
            self.height = 960;
            self.width = 540;
            switch (self.videoConfigurations.videoBitRate) {
                case WXM_VIDEO_BITRATE_LOW:
            ...
            break;
        case WXM_VIDEO_RESOLUTION_SUPER_HIGH:
            self.height = 1280;
            self.width = 720;
            switch (self.videoConfigurations.videoBitRate) {
                case WXM_VIDEO_BITRATE_LOW:
